...
def _stellar_sed_photons_s_nm(
    *,
    ...
    # Clip to the WALTzER range shown in the figures
...
def _apply_throughput_to_sed_photons_s_nm(
    ...
    *,
    ...
    """
    Convert telescope-entrance SED photon rate density to *detected* photon rate density.
    """
...
def _plot_nuv_vis_nir(
    ...
    # Use the *actual* detector coverage from the ETC pickle so the plotted
    ...
    # NIR is a single broad photometric band plotted as one point.
...
def _plot_nuv_vis_nir_realistic(
    ...
    *,
    ...
    # NIR photometry point (with xerr showing the band width)
    if np.isfinite(nir_flux) and nir_flux > 0:
        ax.errorbar(
            [nir_wl_nm],
            [nir_flux],
    ...
    # Mark where the uncertainty band needed clipping (cap)
    y_strip = 0.02
    if np.any(nuv_capped):
    ...
    if show_xticklabels:
        ...
        # keep unique + sorted within bounds
    ...
    # Annotation placement (same logic as compressed)
    if ann_side not in {"left", "right", "center"}:
        ann_side = "left"
    if ann_side == "left":
        ax_x, ax_ha = 0.02, "left"
    elif ann_side == "right":
        ax_x, ax_ha = 0.98, "right"
    else:
        ax_x, ax_ha = 0.50, "center"
    ax.text(
        ax_x,
